...
WURTH_DEV_DIR=$HOME/geo-int/mapstore/mapstore-v1
...
sudo mv $WURTH_DEV_DIR/web/target/map-v1.war $WURTH_DEV_DIR/web/target/wurth.war
sudo cp $WURTH_DEV_DIR/web/target/wurth.war $WEBAPPS_DIR/
...
if [ -d "$WEBAPPS_DIR/wurth" ]; then
    ...
    sudo cp $SETUPS_DIR/geostore-datasource-ovr.properties $WEBAPPS_DIR/wurth/WEB-INF/classes/


    # Delete & Xerces with new one
    sudo rm $WEBAPPS_DIR/wurth/WEB-INF/lib/xercesImpl-2.6.2.jar
    sudo cp $SETUPS_DIR/xercesImpl-2.9.1.jar $WEBAPPS_DIR/wurth/WEB-INF/lib/
...
sudo mv $WURTH_DEV_DIR/web/target/wurth.war $WURTH_DEV_DIR/web/target/map-v1.war
